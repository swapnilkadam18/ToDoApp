...
@HiltViewModel
public class AddTodoViewModel extends ViewModel implements ViewModelCallback {
    ...
    public MutableLiveData<Boolean> onAddSuccess = new MutableLiveData<>();
    ...
    public void addToDb(String inputTitle, String inputDesc, String inputTime,
                        String inputDate, boolean switchVal) {
        if(inputTitle.isEmpty() || inputDesc.isEmpty() || inputDate.isEmpty()
                || inputTime.isEmpty()){
            observeError.setValue("Fields cannot be empty");
        }else{
            showLoader.setValue(visible);
            repo.saveTodo(this, inputTitle,inputDesc,inputTime,
                    inputDate,switchVal);
        }
    }
